...
![Screenshot 2025-03-04 at 7 16 48 PM](DOCS/1.png)
...
1. **VPC (Virtual Private Cloud)**
    - A VPC named `chiradev-vpc` with CIDR `10.0.0.0/16`.
    - Two public subnets (`chiradev-subnet-0` and `chiradev-subnet-1`).
    - An Internet Gateway (`chiradev-igw`) for external connectivity.
    - A Route Table (`chiradev-route-table`) with a default route to the internet.

2. **Security Groups**
    - `chiradev-cluster-sg`: Allows all outbound traffic.
    - `chiradev-node-sg`: Allows all inbound and outbound traffic.

3. **IAM Roles**
    - `chiradev-cluster-role` with `AmazonEKSClusterPolicy`.
    - `chiradev-node-group-role` with policies for worker nodes.

4. **EKS Cluster and Node Group**
    - `chiradev-cluster`: A managed EKS cluster.
    - `chiradev-node-group`: A node group with three `t2.medium` instances.
...
pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/0019-KDU/boardgame-corp-pipeline.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t chiradev/boardgame-app:latest ."
                    }
                }
            }
        }
        ...
        stage('Docker Image Scanner') {
            steps {
                sh 'trivy image --format table -o trivy-image-report.html chiradev/boardgame:latest'
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push chiradev/boardgame-app:latest"
                    }
                }
            }
        }
        ...
        stage('Deploy to the k8s') {
            steps {
                withKubeConfig(caCertificate: '', clusterName: 'chiradev-cluster', contextName: '', credentialsId: 'k8-cred', namespace: 'webapps', restrictKubeConfigAccess: false, serverUrl: 'https://64F3C00D242397024A7FFC4BA5E17651.gr7.us-east-1.eks.amazonaws.com') {
        ...
        always {
            script {
                ...
                emailext (
                    ...
                    body: body,
                    to: 'chirantharavishka@gmail.com',
